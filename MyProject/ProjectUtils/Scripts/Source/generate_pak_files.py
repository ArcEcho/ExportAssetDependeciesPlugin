...
def GeneratePak(outputPakFileDir, targetPackagePathRootDir, longPackageName,  inShouldPackTo64KB = False):
    ...
    filesToWrite = ""
    filesToWrite = longPackageName.replace('/Game', "Content" )
    filesToWrite = os.path.normpath(os.path.join(targetPackagePathRootDir , filesToWrite))
    filesToWrite = filesToWrite + '.*'

    pakFilename = GetSha1OfLongPackageName(longPackageName)
...
if __name__ == "__main__":
    ...
    projectUtilsDir =  os.path.join(GetTargetPorjectRootDir(), "Saved\\ProjectUtils\\")
    if os.path.exists(projectUtilsDir):
        subprocess.check_call("rd /q /s {}".format(projectUtilsDir), shell = True)
    
    subprocess.check_call("md {}".format(projectUtilsDir), shell = True)

    generatePakLogFilepath = os.path.join(projectUtilsDir, "GeneratePaks.log")
    ...
    with open(generatePakLogFilepath, 'w') as generatePakLogFileHandle:
        with open(assetDependenciesFileFilpath, 'r') as assetDependenciesFileHandle:
            ...
            for (targetPackage,dependentData) in  data.items():
                ...
                targetOutputDir = os.path.join(projectUtilsDir, sha1OfLongTargetPackageName)
                ...
                bShouldGenerateBatchPak = False 
                if bShouldGenerateBatchPak:
